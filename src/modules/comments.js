...
import { createPromiseSaga, handleAsyncActions, reducerUtils } from '../util/async.utill';
...
export default function comments(state = initialState, action) {
  if (action.type.startsWith(GET_COMMENTS)) return handleAsyncActions(GET_COMMENTS, 'comments')(state, action);
  if (action.type.startsWith(SAVE_COMMENT)) return handleAsyncActions(SAVE_COMMENT, 'comment')(state, action);
  if (action.type.startsWith(UPDATE_COMMENT)) return handleAsyncActions(UPDATE_COMMENT, 'comment')(state, action);
  if (action.type.startsWith(DELETE_COMMENT)) return handleAsyncActions(DELETE_COMMENT, 'comment')(state, action);
